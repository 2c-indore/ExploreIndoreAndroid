package org.kathmandulivinglabs.preparepokhara.Customclass;
...
public class KllclusterRenderer<T extends CustomClusterItem> implements ClusterRenderer<T> {
    ...
    private final CustomClusterManagerPlugin<T> mClusterManagerPlugin;
     ...
     */
    private Set<KllclusterRenderer.MarkerWithPosition> mMarkers = Collections.newSetFromMap(
            new ConcurrentHashMap<KllclusterRenderer.MarkerWithPosition, Boolean>());
     ...
     */
    private KllclusterRenderer.MarkerCache<T> mMarkerCache = new KllclusterRenderer.MarkerCache<T>();
    ...
    private final KllclusterRenderer.ViewModifier mViewModifier = new KllclusterRenderer.ViewModifier();
    ...
    public KllclusterRenderer(Context context, MapboxMap map, CustomClusterManagerPlugin<T> clusterManagerPlugin) {
        mMap = map;
        mAnimate = true;
        mDensity = context.getResources().getDisplayMetrics().density;
        mIconGenerator = new IconGenerator(context);
        mIconGenerator.setContentView(makeSquareTextView(context));
        mIconGenerator.setTextAppearance(com.mapbox.mapboxsdk.plugins.cluster.R.style.amu_ClusterIcon_TextAppearance);
    ...
    private SquareTextView makeSquareTextView(Context context) {
        ...
        squareTextView.setId(com.mapbox.mapboxsdk.plugins.cluster.R.id.amu_text);
    ...
    private class ViewModifier extends Handler {
        ...
        private KllclusterRenderer.RenderTask mNextClusters = null;
        ...
        public void handleMessage(Message msg) {
            ...
            KllclusterRenderer.RenderTask renderTask;
            synchronized (this) {
        ...
        public void queue(Set<? extends Cluster<T>> clusters) {
            synchronized (this) {
                ...
                mNextClusters = new KllclusterRenderer.RenderTask(clusters);
            }
            sendEmptyMessage(RUN_TASK);
        }
    }
    ...
    /**
     * Transforms the current view (represented by KllclusterRenderer.mClusters and KllclusterRenderer.mZoom) to a
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(KllclusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }

            final KllclusterRenderer.MarkerModifier markerModifier = new KllclusterRenderer.MarkerModifier();

            final float zoom = mMapZoom;
            final boolean zoomingIn = zoom > mZoom;
            final float zoomDelta = zoom - mZoom;

            final Set<KllclusterRenderer.MarkerWithPosition> markersToRemove = mMarkers;
            ...
            if (KllclusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : KllclusterRenderer.this.mClusters) {
            ...
            final Set<KllclusterRenderer.MarkerWithPosition> newMarkers = Collections.newSetFromMap(
                    new ConcurrentHashMap<KllclusterRenderer.MarkerWithPosition, Boolean>());
            for (Cluster<T> c : clusters) {
                ...
                if (zoomingIn && onScreen && SHOULD_ANIMATE) {
                    ...
                    if (closest != null && mAnimate) {
                        ...
                        markerModifier.add(true, new KllclusterRenderer.CreateMarkerTask(c, newMarkers, animateTo));
                    } else {
                        markerModifier.add(true, new KllclusterRenderer.CreateMarkerTask(c, newMarkers, null));
                    }
                } else {
                    markerModifier.add(onScreen, new KllclusterRenderer.CreateMarkerTask(c, newMarkers, null));
                }
            }
            ...
            for (final KllclusterRenderer.MarkerWithPosition marker : markersToRemove) {
            ...
            mMarkers = newMarkers;
            KllclusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
        ...
        private Queue<KllclusterRenderer.CreateMarkerTask> mCreateMarkerTasks = new LinkedList<KllclusterRenderer.CreateMarkerTask>();
        private Queue<KllclusterRenderer.CreateMarkerTask> mOnScreenCreateMarkerTasks = new LinkedList<KllclusterRenderer.CreateMarkerTask>();
        ...
        private Queue<KllclusterRenderer.AnimationTask> mAnimationTasks = new LinkedList<KllclusterRenderer.AnimationTask>();
         ...
         */
        public void add(boolean priority, KllclusterRenderer.CreateMarkerTask c) {
            lock.lock();
         ...
         */
        public void animate(KllclusterRenderer.MarkerWithPosition marker, LatLng from, LatLng to) {
            lock.lock();
            mAnimationTasks.add(new KllclusterRenderer.AnimationTask(marker, from, to));
            lock.unlock();
        }
        ...
        public void animateThenRemove(KllclusterRenderer.MarkerWithPosition marker, LatLng from, LatLng to) {
            lock.lock();
            KllclusterRenderer.AnimationTask animationTask = new KllclusterRenderer.AnimationTask(marker, from, to);
    ...
    private class CreateMarkerTask {
        ...
        private final Set<KllclusterRenderer.MarkerWithPosition> newMarkers;
         ...
         */
        public CreateMarkerTask(Cluster<T> c, Set<KllclusterRenderer.MarkerWithPosition> markersAdded, LatLng animateFrom) {
            this.cluster = c;
            this.newMarkers = markersAdded;
            this.animateFrom = animateFrom;
        }

        private void perform(KllclusterRenderer.MarkerModifier markerModifier) {
            // Don't show small clusters. Render the markers inside, instead.
            if (!shouldRenderAsCluster(cluster)) {
                for (T item : cluster.getItems()) {
                    Marker marker = mMarkerCache.get(item);
                    KllclusterRenderer.MarkerWithPosition markerWithPosition;
                    if (marker == null) {
                        ...
                        markerWithPosition = new KllclusterRenderer.MarkerWithPosition(marker);
    ...
    private static class MarkerWithPosition {
        ...
        public boolean equals(Object other) {
            if (other instanceof KllclusterRenderer.MarkerWithPosition) {
                return marker.equals(((KllclusterRenderer.MarkerWithPosition) other).marker);
            }
            return false;
        }
    ...
    private class AnimationTask extends AnimatorListenerAdapter implements ValueAnimator.AnimatorUpdateListener {
        private final KllclusterRenderer.MarkerWithPosition markerWithPosition;
        private final Marker marker;
        ...
        private AnimationTask(KllclusterRenderer.MarkerWithPosition markerWithPosition, LatLng from, LatLng to) {
